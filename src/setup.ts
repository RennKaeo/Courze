...
export async function setup(
  ...
  if (!isBareMode() || messagingSocketPath !== undefined) {
    ...
    // and $COURSE_CODE_MESSAGING_SOCKET is exported before any hook
  ...
  // When COURSE_CODE_SYNC_PLUGIN_INSTALL is set, skip all plugin prefetch.
  ...
  const skipPluginPrefetch =
    (getIsNonInteractiveSession() &&
      isEnvTruthy(process.env.COURSE_CODE_SYNC_PLUGIN_INSTALL)) ||
  ...
  if (
    ...
    if (
      ...
      !isEnvTruthy(process.env.COURSE_CODE_BUBBLEWRAP)
    ) {
      // biome-ignore lint/suspicious/noConsole:: intentional console output
      console.error(
