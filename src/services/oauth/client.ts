...
export async function exchangeCodeForTokens(
  ...
  logEvent('courze_oauth_token_exchange_success', {})
...
export async function refreshOAuthToken(
  ...
  try {
    ...
    logEvent('courze_oauth_token_refresh_success', {})
    ...
    // COURSE_CODE_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
    ...
    const responseBody =
      axios.isAxiosError(error) && error.response?.data
        ...
        : undefined
    logEvent('courze_oauth_token_refresh_failure', {
...
export async function fetchAndStoreUserRoles(
  ...
  logEvent('courze_oauth_roles_stored', {
    org_role:
...
export async function fetchProfileInfo(accessToken: string): Promise<{
  ...
  logEvent('courze_oauth_profile_fetch_success', {})
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.COURSE_CODE_ACCOUNT_UUID
  const envUserEmail = process.env.COURSE_CODE_USER_EMAIL
  const envOrganizationUuid = process.env.COURSE_CODE_ORGANIZATION_UUID
