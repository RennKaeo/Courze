...
/**
 * Thresholds from courze_onyx_plover. The enabled gate lives in config.ts
...
export function initAutoDream(): void {
  ...
  runner = async function runAutoDream(context, appendSystemMessage) {
    ...
    logEvent('courze_auto_dream_fired', {
...
${sessionIds.map(id => `- ${id}`).join('\n')}`
      ...
      logEvent('courze_auto_dream_completed', {
      ...
      logEvent('courze_auto_dream_failed', {})
