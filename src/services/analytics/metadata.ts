 ...
 * pattern at each courze_tool_use_* call site.
 */
...
export type EventMetadata = {
  ...
  agentId?: string // COURSE_CODE_AGENT_ID (format: agentName@teamName) or subagent UUID
  parentSessionId?: string // COURSE_CODE_PARENT_SESSION_ID (team lead's session)
  ...
  observerMode?: 'backseat' | 'skillcoach' | 'both' // Which observer classifiers are gated on (internal-only; for BQ cohort splits on courze_backseat_* events)
}
...
const buildEnvContext = memoize(async (): Promise<EnvContext> => {
  ...
  return {
    ...
    // the truth. COURSE_CODE_HOST_PLATFORM still overrides for container/remote.
    platformRaw: process.env.COURSE_CODE_HOST_PLATFORM || process.platform,
    arch: env.arch,
    ...
    isClaudeCodeRemote: isEnvTruthy(process.env.COURSE_CODE_REMOTE),
    isLocalAgentMode: process.env.COURSE_CODE_ENTRYPOINT === 'local-agent',
    isConductor: env.isConductor(),
    ...(process.env.COURSE_CODE_REMOTE_ENVIRONMENT_TYPE && {
      remoteEnvironmentType: process.env.COURSE_CODE_REMOTE_ENVIRONMENT_TYPE,
    }),
    // Gated by feature flag to prevent leaking "coworkerType" string in external builds
    ...(feature('COWORKER_TYPE_TELEMETRY')
      ? process.env.COURSE_CODE_COWORKER_TYPE
        ? { coworkerType: process.env.COURSE_CODE_COWORKER_TYPE }
        : {}
      : {}),
    ...(process.env.COURSE_CODE_CONTAINER_ID && {
      claudeCodeContainerId: process.env.COURSE_CODE_CONTAINER_ID,
    }),
    ...(process.env.COURSE_CODE_REMOTE_SESSION_ID && {
      claudeCodeRemoteSessionId: process.env.COURSE_CODE_REMOTE_SESSION_ID,
    }),
    ...(process.env.COURSE_CODE_TAGS && {
      tags: process.env.COURSE_CODE_TAGS,
    }),
    isGithubAction: isEnvTruthy(process.env.GITHUB_ACTIONS),
    isClaudeCodeAction: isEnvTruthy(process.env.COURSE_CODE_ACTION),
    ...
    ...(isEnvTruthy(process.env.GITHUB_ACTIONS) && {
      ...
      githubActionRef: process.env.GITHUB_ACTION_PATH?.includes(
        ...
        ? process.env.GITHUB_ACTION_PATH.split('course-code-action/')[1]
        : undefined,
    }),
...
export async function getEventMetadata(
  ...
  const metadata: EventMetadata = {
    ...
    envContext,
    ...(process.env.COURSE_CODE_ENTRYPOINT && {
