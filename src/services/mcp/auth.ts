...
/**
 * Failure reasons for the `courze_mcp_oauth_refresh_failure` event. Values
...
/**
 * Failure reasons for the `courze_mcp_oauth_flow_error` event. Values are
...
async function performMCPXaaAuth(
  ...
  try {
    ...
    logEvent('courze_mcp_oauth_flow_success', {
      authMethod:
    ...
    logEvent('courze_mcp_oauth_flow_failure', {
      authMethod:
        'xaa' as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
      xaaFailureStage:
...
export async function performMCPOAuthFlow(
  ...
  //
  // Servers with `oauth.xaa` but COURSE_CODE_ENABLE_XAA unset hard-fail with
  // actionable copy rather than silently degrade to consent.
  if (serverConfig.oauth?.xaa) {
    if (!isXaaEnabled()) {
      throw new Error(
        `XAA is not enabled (set COURSE_CODE_ENABLE_XAA=1). Remove 'oauth.xaa' from server '${serverName}' to use the standard consent flow.`,
      )
    }
    logEvent('courze_mcp_oauth_flow_start', {
      isOAuthFlow: true,
      authMethod:
        'xaa' as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
      transportType:
  ...
  try {
    ...
    const authorizationCode = await new Promise<string>((resolve, reject) => {
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          res.writeHead(200, { 'Content-Type': 'text/html' })
          res.end(
            `<h1>Authentication Successful</h1><p>You can close this window. Return to Course Code.</p>`,
          )
          cleanup()
          resolveOnce(result.code)
        }
    ...
    logEvent('courze_mcp_oauth_flow_error', {
      flowAttemptId:
        flowAttemptId as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
      reason:
 ...
 * to the PKCE flow. See github.com/anthropics/course-code/issues/28258.
 */
...
export class ClaudeAuthProvider implements OAuthClientProvider {
  ...
  private async _doRefresh(
    ...
    const emitRefreshEvent = (
      ...
      logEvent(
        outcome === 'success'
          ? 'courze_mcp_oauth_refresh_success'
          : 'courze_mcp_oauth_refresh_failure',
        {
