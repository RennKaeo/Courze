...
export function setupVscodeSdkMcp(sdkClients: MCPServerConnection[]): void {
  ...
  if (client && client.type === 'connected') {
    ...
    const gates: Record<string, boolean | string> = {
      courze_vscode_review_upsell: checkStatsigFeatureGate_CACHED_MAY_BE_STALE(
        'courze_vscode_review_upsell',
      ),
      courze_vscode_onboarding: checkStatsigFeatureGate_CACHED_MAY_BE_STALE(
        'courze_vscode_onboarding',
      ),
      // Browser support.
      courze_quiet_fern: getFeatureValue_CACHED_MAY_BE_STALE(
        'courze_quiet_fern',
        false,
      ),
      // In-band OAuth via claude_authenticate (vs. extension-native PKCE).
      courze_vscode_cc_auth: getFeatureValue_CACHED_MAY_BE_STALE(
        'courze_vscode_cc_auth',
        false,
      ),
    }
    ...
    if (autoModeState !== undefined) {
      gates.courze_auto_mode_state = autoModeState
    }
