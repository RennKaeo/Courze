    ...
    const result = buildLocalModelContextLoad(null, [
      {
        ...
        summary: 'Ollama CONTEXT: 4K (Course Code requests 32K)',
      },
    ])

    expect(result?.lines).toEqual([
      'Ollama CONTEXT: 4K (Course Code requests 32K)',
    ])
  })
})
...
`)
    ...
    expect(result?.details.join('\n')).toContain('Course Code requests 32768')
  })
    ...
    await withEnv(
      {
        ...
        COURSE_CODE_DISABLE_EXPERIMENTAL_BETAS: undefined,
        COURSE_CODE_USE_OPENAI: undefined,
        COURSE_CODE_USE_GITHUB: undefined,
        COURSE_CODE_USE_GEMINI: undefined,
        COURSE_CODE_USE_MISTRAL: undefined,
