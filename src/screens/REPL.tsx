...
export function REPL({
  ...
  const titleDisabled = useMemo(() => isEnvTruthy(process.env.COURSE_CODE_DISABLE_TERMINAL_TITLE), []);
  const disableVirtualScroll = useMemo(() => isEnvTruthy(process.env.COURSE_CODE_DISABLE_VIRTUAL_SCROLL), []);
  const disableMessageActions = feature('MESSAGE_ACTIONS') ?
    // biome-ignore lint/correctness/useHookAtTopLevel: feature() is a compile-time constant
    useMemo(() => isEnvTruthy(process.env.COURSE_CODE_DISABLE_MESSAGE_ACTIONS), []) : false;
  ...
  // from COURSE_CODE_NO_FLICKER=0 (which is process-lifetime) — this is
  ...
  const terminalTitle = sessionTitle ?? agentTitle ?? haikuTitle ?? 'Course Code';
  ...
  useEffect(() => {
    ...
    setMessages(prev => [...prev, createSystemMessage(`Worktree creation took ${secs}s. For large repos, set \`worktree.sparsePaths\` in .course/settings.json to check out only the directories you need — e.g. \`{"worktree": {"sparsePaths": ["src", "packages/foo"]}}\`.`, 'info')]);
  }, [setMessages]);
  ...
  // That write storm is the primary trigger for ~/.course.json corruption
  // (GH #3117).
  ...
  useEffect(() => {
    ...
    const idleThresholdMs = Number(process.env.COURSE_CODE_IDLE_THRESHOLD_MINUTES ?? 75) * 60_000;
  ...
  useEffect(() => {
    const handleSuspend = () => {
      ...
      process.stdout.write(`\nCourse Code has been suspended. Run \`fg\` to bring Course Code back.\nNote: ctrl + z now suspends Course Code, ctrl + _ undoes input.\n`);
    };
    const handleResume = () => {
