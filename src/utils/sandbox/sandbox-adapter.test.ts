...
describe('convertToSandboxRuntimeConfig', () => {
  ...
  beforeEach(async () => {
    ...
    tempRoot = await mkdtemp(join(tmpdir(), 'course-sandbox-adapter-'))
  ...
  test('denies canonical Course Code settings files in changed cwd', () => {
    const config = convertToSandboxRuntimeConfig({} as SettingsJson)

    expect(config.filesystem.denyWrite).toContain(
      resolve(activeCwd, '.course', 'settings.json'),
    )
    expect(config.filesystem.denyWrite).toContain(
      resolve(activeCwd, '.course', 'settings.local.json'),
    )
    expect(config.filesystem.denyWrite).toContain(
