...
export async function toolToAPISchema(
  ...
  // COURSE_CODE_DISABLE_EXPERIMENTAL_BETAS is the kill switch for beta API
  ...
  if (isEnvTruthy(process.env.COURSE_CODE_DISABLE_EXPERIMENTAL_BETAS)) {
...
function logStripOnce(stripped: string[]): void {
  ...
  loggedStrip = true
  logForDebugging(
    `[betas] Stripped from tool schemas: [${stripped.join(', ')}] (COURSE_CODE_DISABLE_EXPERIMENTAL_BETAS=1)`,
  )
}
