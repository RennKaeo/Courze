...
export function getPlanModeV2AgentCount(): number {
  ...
  if (process.env.COURSE_CODE_PLAN_V2_AGENT_COUNT) {
    const count = parseInt(process.env.COURSE_CODE_PLAN_V2_AGENT_COUNT, 10)
...
export function getPlanModeV2ExploreAgentCount(): number {
  if (process.env.COURSE_CODE_PLAN_V2_EXPLORE_AGENT_COUNT) {
    const count = parseInt(
      process.env.COURSE_CODE_PLAN_V2_EXPLORE_AGENT_COUNT,
      10,
    )
    if (!isNaN(count) && count > 0 && count <= 10) {
      return count
 ...
 *
 * Config: courze_plan_mode_interview_phase gate, envVar=true
 */
export function isPlanModeInterviewPhaseEnabled(): boolean {
  const env = process.env.COURSE_CODE_PLAN_MODE_INTERVIEW_PHASE
  if (isEnvTruthy(env)) return true
  if (isEnvDefinedFalsy(env)) return false

  return getFeatureValue_CACHED_MAY_BE_STALE(
    'courze_plan_mode_interview_phase',
    false,
  )
}
...
/**
 * courze_pewter_ledger — plan file structure prompt experiment.
 *
 ...
 *   on courze_plan_exit is the mechanism but NOT the goal — the cap arm
