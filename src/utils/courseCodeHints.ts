/**
 * Courze hints protocol.
 *
 * CLIs and SDKs running under Courze can emit a self-closing
 * `<course-code-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/course-code-hints.md for the vendor-facing spec.
 */

import { logForDebugging } from './debug.js'
import { createSignal } from './signal.js'

export type CourseCodeHintType = 'plugin'

export type CourseCodeHint = {
  /** Spec version declared by the emitter. Unknown versions are dropped. */
  v: number
  /** Hint discriminator. v1 defines only `plugin`. */
  type: CourseCodeHintType
  /**
 ...
 */
const HINT_TAG_RE = /^[ \t]*<course-code-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
 ...
 */
export function extractCourseCodeHints(
  output: string,
  command: string,
): { hints: CourseCodeHint[]; stripped: string } {
  // Fast path: no tag open sequence → no work, no allocation.
  if (!output.includes('<course-code-hint')) {
    return { hints: [], stripped: output }
  }

  const sourceCommand = firstCommandToken(command)
  const hints: CourseCodeHint[] = []
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[courseCodeHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[courseCodeHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[courseCodeHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as CourseCodeHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: CourseCodeHint | null = null
...
export function setPendingHint(hint: CourseCodeHint): void {
...
export function getPendingHintSnapshot(): CourseCodeHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetCourseCodeHintStore(): void {
  pendingHint = null
