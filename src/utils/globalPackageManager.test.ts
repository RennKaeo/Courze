...
describe('getGlobalInstallArgs', () => {
  const spec = '@renskaeo/courze@latest'
  ...
  test('passes an explicit version spec through unchanged', () => {
    expect(getGlobalInstallArgs('npm', '@renskaeo/courze@1.2.3')).toEqual([
      'install',
      '-g',
      '@renskaeo/courze@1.2.3',
    ])
  })
})
...
describe('selectOwningPackageManager', () => {
  ...
  test('matches the package manager whose root contains the binary', () => {
    expect(
      selectOwningPackageManager(
        '/home/u/.local/share/pnpm/global/5/node_modules/@renskaeo/courze/cli.js',
        [
          { pm: 'npm', root: '/usr/local/lib/node_modules' },
          {
            pm: 'pnpm',
  ...
  test('most specific (longest) root wins when roots are nested', () => {
    ...
    expect(
      selectOwningPackageManager('/opt/pm/node_modules/bun/global/course', [
  ...
  test('does not match a sibling directory sharing a prefix', () => {
    ...
    expect(
      selectOwningPackageManager('/a/node_modules-other/course/cli.js', [
  ...
  test('ignores candidates with an empty root', () => {
    expect(
      selectOwningPackageManager('/usr/lib/node_modules/course', [
        { pm: 'yarn', root: '' },
