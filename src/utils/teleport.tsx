...
export async function validateGitState(): Promise<void> {
  ...
  if (!isClean) {
    logEvent('courze_teleport_error_git_not_clean', {});
...
async function checkoutBranch(branchName: string): Promise<void> {
  ...
  if (checkoutCode !== 0) {
    logEvent('courze_teleport_error_branch_checkout_failed', {});
...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    ...
    if (!accessToken) {
      logEvent('courze_teleport_resume_error', {
    ...
    switch (repoValidation.status) {
      ...
      case 'not_in_repo':
        {
          logEvent('courze_teleport_error_repo_not_in_git_dir_sessions_api', {
          ...
          throw new TeleportOperationError(`You must run course --teleport ${sessionId} from a checkout of ${notInRepoDisplay}.`, chalk.red(`You must run course --teleport ${sessionId} from a checkout of ${chalk.bold(notInRepoDisplay)}.\n`));
        }
      case 'mismatch':
        {
          logEvent('courze_teleport_error_repo_mismatch_sessions_api', {
            sessionId: sessionId as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS
          });
          ...
          throw new TeleportOperationError(`You must run course --teleport ${sessionId} from a checkout of ${sessionDisplay}.\nThis repo is ${currentDisplay}.`, chalk.red(`You must run course --teleport ${sessionId} from a checkout of ${chalk.bold(sessionDisplay)}.\nThis repo is ${chalk.bold(currentDisplay)}.\n`));
        }
      case 'error':
...
async function handleTeleportPrerequisites(root: Root, errorsToIgnore?: Set<TeleportLocalErrorType>): Promise<void> {
  ...
  if (errors.size > 0) {
    ...
    logEvent('courze_teleport_errors_detected', {
    ...
    await new Promise<void>(resolve => {
      root.render(<AppStateProvider>
          <KeybindingSetup>
            ...
            logEvent('courze_teleport_errors_resolved', {
              error_types: Array.from(errors).join(',') as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS
            });
            void resolve();
          }} />
...
export async function teleportFromSessionsAPI(sessionId: string, orgUUID: string, accessToken: string, onProgress?: TeleportProgressCallback, sessionData?: SessionResource): Promise<TeleportRemoteResponse> {
  ...
  try {
    ...
    if (axios.isAxiosError(error) && error.response?.status === 404) {
      logEvent('courze_teleport_error_session_not_found_404', {
        sessionId: sessionId as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS
      });
...
export async function teleportToRemote(options: {
   ...
   * environmentId is set, COURSE_CODE_OAUTH_TOKEN is auto-injected from the
  ...
  try {
    ...
    if (options.environmentId) {
      ...
      const envVars = {
        COURSE_CODE_OAUTH_TOKEN: accessToken,
      ...
      if (options.useBundle) {
        ...
        logEvent('courze_teleport_bundle_mode', {
    ...
    const bundleSeedGateOn = !options.skipBundle && gitRoot !== null && (isEnvTruthy(process.env.CCR_ENABLE_BUNDLE) || (await checkGate_CACHED_OR_BLOCKING('courze_ccr_bundle_seed_enabled')));
    ...
    logEvent('courze_teleport_source_decision', {
      reason: sourceReason as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
