...
const cwd = '/workspace/course'

function withTempTranscript(
  entries: Array<Record<string, unknown> | string>,
  fn: (path: string) => Promise<void>,
) {
  const dir = mkdtempSync(join(tmpdir(), 'course-task-report-'))
...
<summary>Background command "bun test src/utils/reportTask.test.ts" completed (exit code 0)</summary>
    ...
    await withTempTranscript(
      [
        ...
        },
        {
          ...
          worktreeSession: {
            originalCwd: cwd,
            worktreePath: '/workspace/course-report',
            worktreeName: 'course-report',
        ...
        },
        {
          ...
          sessionId,
          prNumber: 456,
          prUrl: 'https://github.com/RennKaeo/Courze/pull/456',
          prRepository: 'RennKaeo/Courze',
          timestamp: '2026-06-27T08:01:00.000Z',
        },
        userMessage(
          '00000000-0000-4000-8000-000000000008',
          'Update src/report.ts for https://github.com/RennKaeo/Courze/issues/123.',
          '2026-06-27T08:00:00.000Z',
        ),
        assistantToolMessage(
      ...
      async transcriptPath => {
        ...
        expect(report.branch.pullRequest).toEqual({
          number: 456,
          repository: 'RennKaeo/Courze',
          url: 'https://github.com/RennKaeo/Courze/pull/456',
        })
        ...
        expect(report.linkedReferences).toEqual([
          {
            ...
            number: 123,
            repository: 'RennKaeo/Courze',
            url: 'https://github.com/RennKaeo/Courze/issues/123',
          },
          {
            kind: 'pull_request',
            number: 456,
  ...
  test('normalizes Windows-style tool paths before merging with git paths', async () => {
    const windowsCwd = 'C:\\workspace\\course'
      ...
      [
        ...
        assistantToolMessage(
          '00000000-0000-4000-8000-000000000041',
          {
            ...
            name: 'Edit',
            input: {
              file_path: 'C:\\workspace\\course\\src\\report.ts',
              old_string: 'old',
              new_string: 'new',
            },
          },
        ...
        assistantToolMessage(
          '00000000-0000-4000-8000-000000000043',
          {
            ...
            name: 'Edit',
            input: {
              file_path: 'C:\\workspace\\course\\..fixtures\\report.ts',
              old_string: 'old',
              new_string: 'new',
            },
          },
    ...
    await withTempTranscript(
      [
        ...
        toolResultMessage(
          ...
          'tool-read',
          'File body mentions https://github.com/RennKaeo/Courze/issues/999.',
          '2026-06-27T08:01:01.000Z',
        ),
      ],
      async transcriptPath => {
  ...
  test('formats markdown for changed files and branch metadata', async () => {
    const reportCwd = join(tmpdir(), 'course')
    const worktreePath = join(tmpdir(), 'course-report')
    ...
    await withTempTranscript(
      ...
      async transcriptPath => {
        ...
        expect(markdown).toContain(
          '- Pull request: [#456](<https://github.com/RennKaeo/Courze/pull/456>) (`RennKaeo/Courze`)',
        )
        expect(markdown).toContain(`- \`${reportSourcePath}\` (git, tool)`)
        expect(markdown).toContain(`- \`${reportTestPath}\` (git)`)
        expect(markdown).toContain(
          '- pull_request: [#456](<https://github.com/RennKaeo/Courze/pull/456>) (`RennKaeo/Courze`)',
        )
      },
    )
  })
  ...
  test('sanitizes linked reference markdown URLs', async () => {
    await withTempTranscript(
      ...
      async transcriptPath => {
        ...
        report.linkedReferences = [
          {
            ...
            number: 321,
            repository: 'RennKaeo/Courze',
            url: 'https://github.com/RennKaeo/Courze/issues/321?label=a(b)',
          },
          {
            kind: 'pull_request',
            number: 9,
        ...
        expect(markdown).toContain(
          '- issue: [#321](<https://github.com/RennKaeo/Courze/issues/321?label=a(b)>) (`RennKaeo/Courze`)',
        )
  ...
  test('formats markdown with redacted secrets and truncated command output', async () => {
    const secret = 'sk-course-test-secret'
  ...
  test('parses markdown report options through the CLI command', async () => {
    ...
    await program.parseAsync(
      ['node', 'course', 'report', '--markdown', '--transcript', 'session.jsonl'],
      { from: 'node' },
    )
  ...
  test('parses json report options through the CLI command', async () => {
    ...
    await program.parseAsync(
      ['node', 'course', 'report', '--json', '--session', sessionId, '--out', 'task-report.json'],
      { from: 'node' },
    )
  ...
  test('rejects missing and conflicting report output flags through the CLI command', async () => {
    ...
    await missingProgram.parseAsync(['node', 'course', 'report'], {
      from: 'node',
    })
    ...
    await expect(
      conflictingProgram.parseAsync(
        ['node', 'course', 'report', '--json', '--markdown'],
        { from: 'node' },
      ),
  ...
  test('omits dirty status when git status cannot be collected', async () => {
    const repoDir = join(homedir(), 'course-task-report-git-repo')
    const calls: string[] = []
    ...
    expect(metadata).toEqual({
      status: 'available',
      cwd: join('~', 'course-task-report-git-repo'),
      branch: 'feat/report',
      head: '13cf30afa469',
      changedFiles: [],
      error: 'status failed',
