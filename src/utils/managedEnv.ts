 ...
 * COURSE_CODE_PROVIDER_MANAGED_BY_HOST in spawn env), strip
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.COURSE_CODE_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.course.json) is user-controlled. In CCD mode,
  ...
  // flagSettings env applied. Eligibility reads COURSE_CODE_USE_BEDROCK,
  ...
  // when COURSE_CODE_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * COURSE_CODE_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
