...
export function getUserAgent(): string {
  ...
  return `claude-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.COURSE_CODE_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}

export function getMCPUserAgent(): string {
  const parts: string[] = []
  if (process.env.COURSE_CODE_ENTRYPOINT) {
    parts.push(process.env.COURSE_CODE_ENTRYPOINT)
  }
  ...
  return `course-code/${MACRO.VERSION}${suffix}`
}
...
// operators match in robots.txt); the course-code suffix lets them distinguish
