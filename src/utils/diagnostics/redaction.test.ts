...
describe('diagnostic redaction', () => {
  ...
  test('redacts secret-looking values even under harmless field names', () => {
    ...
    const redacted = redactDiagnosticObject({
      ...
      path: `${home}/private/course/src/file.ts`,
    ...
    expect(redacted.path).toBe('~/private/course/src/file.ts')
  ...
  test('redacts Windows-style home paths without matching sibling directories', () => {
    ...
    expect(
      redactHomePath(
        'debug path C:\\Users\\Alice\\AppData\\Roaming\\course',
        home,
      ),
    ).toBe('debug path ~\\AppData\\Roaming\\course')
    expect(redactHomePath('C:\\Users\\AliceOther\\course', home)).toBe(
      'C:\\Users\\AliceOther\\course',
    )
  })
