...
export async function glob(
  ...
  // --no-ignore: don't respect .gitignore (default true, set COURSE_CODE_GLOB_NO_IGNORE=false to respect .gitignore)
  // --hidden: include hidden files (default true, set COURSE_CODE_GLOB_HIDDEN=false to exclude)
  // Note: use || instead of ?? to treat empty string as unset (defaulting to true)
  const noIgnore = isEnvTruthy(process.env.COURSE_CODE_GLOB_NO_IGNORE || 'true')
  const hidden = isEnvTruthy(process.env.COURSE_CODE_GLOB_HIDDEN || 'true')
  const args = [
    '--files',
    '--glob',
    searchPattern,
