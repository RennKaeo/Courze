...
function getCliBinaryName(): string {
  return MACRO.PACKAGE_URL === '@anthropic-ai/course-code'
    ? 'claude'
    : 'course'
}
...
> {
  ...
  const packagesToCheck = ['@anthropic-ai/course-code']
...
export async function detectStaleProjectSettingsPaths(
  ...
  return {
    issue: `Legacy project settings file${stale.length === 1 ? '' : 's'} ${legacyPaths} found, but Course Code reads ${canonicalPaths}`,
    fix: `Move or copy ${legacyPaths} to ${canonicalPaths} if you intended Course Code to use those project settings.`,
  }
}
    ...
    if (!claudeInPath && !validAlias) {
      if (existingAlias) {
        ...
        warnings.push({
          ...
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias ${getCliBinaryName()}="~/.course/local/${getCliBinaryName()}"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Create alias: alias ${getCliBinaryName()}="~/.course/local/${getCliBinaryName()}"`,
        })
      }
    }
  }
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        let uninstallCmd = 'npm -g uninstall @anthropic-ai/course-code'
        if (
          MACRO.PACKAGE_URL &&
