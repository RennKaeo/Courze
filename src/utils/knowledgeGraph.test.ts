...
describe('KnowledgeGraph Global Persistence & RAG', () => {
  ...
  it('persists entities across loads', async () => {
    await addGlobalEntity('tool', 'course', { status: 'alpha' })
    ...
    const entities = Object.values(graph.entities).filter(e => e.name === 'course')
  ...
  it('deduplicates entities and updates attributes', async () => {
    await addGlobalEntity('tool', 'course', { status: 'alpha' })
    await addGlobalEntity('tool', 'course', { status: 'beta', version: '0.6.0' })
