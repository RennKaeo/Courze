 ...
 * - Checks environment kind (COURSE_CODE_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires COURSE_CODE_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.COURSE_CODE_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but COURSE_CODE_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
  ...
  logEvent('courze_file_persistence_started', {
  ...
  try {
    ...
    logEvent('courze_file_persistence_completed', {
...
async function executeBYOCPersistence(
  ...
  if (modifiedFiles.length > FILE_COUNT_LIMIT) {
    ...
    logEvent('courze_file_persistence_limit_exceeded', {
 ...
 * and COURSE_CODE_REMOTE_SESSION_ID.
