...
export function openFileInExternalEditor(
  ...
  if (guiFamily) {
    ...
    if (process.platform === 'win32') {
      // Use cmd /c with argv array — avoids shell:true string injection risk.
      // cmd.exe can execute .cmd/.bat files via CreateProcess when passed as
      // the command to /c. The editor binary + args are passed as separate
      // argv elements so shell metacharacters in editor/filePath are not
      // interpreted.
      child = spawn('cmd.exe', ['/d', '/c', base, ...editorArgs, ...gotoArgv], detachedOpts)
    } else {
  ...
  try {
    ...
    if (process.platform === 'win32') {
      // Use cmd /c with argv array — avoids shell:true string injection risk.
      // cmd.exe builtins like `start` resolve correctly with /c, and args are
      // passed as separate argv elements so shell metacharacters in editor
      // or filePath are not interpreted.
      const syncArgs = [
        ...(useGotoLine ? [`+${line}`, filePath] : [filePath]),
      ]
      result = spawnSync('cmd.exe', ['/d', '/c', base, ...editorArgs, ...syncArgs], syncOpts)
    } else {
