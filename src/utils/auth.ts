...
function isManagedOAuthContext(): boolean {
  return (
    ...
    process.env.COURSE_CODE_ENTRYPOINT === 'claude-desktop'
  )
}
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // local auth-injecting proxy. The launcher sets COURSE_CODE_OAUTH_TOKEN as a
  ...
  if (process.env.ANTHROPIC_UNIX_SOCKET) {
    return !!process.env.COURSE_CODE_OAUTH_TOKEN
  }
  ...
  const hasExternalAuthToken =
    ...
    apiKeyHelper ||
    process.env.COURSE_CODE_API_KEY_FILE_DESCRIPTOR
...
export function getAuthTokenSource() {
  ...
  if (process.env.COURSE_CODE_OAUTH_TOKEN) {
    return { source: 'COURSE_CODE_OAUTH_TOKEN' as const, hasToken: true }
  }
  ...
  if (oauthTokenFromFd) {
    ...
    if (process.env.COURSE_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR) {
      return {
        source: 'COURSE_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR' as const,
        hasToken: true,
      }
    }
    return {
...
export function getAnthropicApiKeyWithSource(
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test') {
    ...
    if (
      !isUsing3PServices() &&
      !apiKeyEnv &&
      !process.env.COURSE_CODE_OAUTH_TOKEN &&
      !process.env.COURSE_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR
    ) {
      throw new Error(
        'ANTHROPIC_API_KEY or COURSE_CODE_OAUTH_TOKEN env var is required',
      )
    }
 ...
 * Uses COURSE_CODE_API_KEY_HELPER_TTL_MS env var if set and valid,
 * otherwise defaults to 5 minutes
 */
export function calculateApiKeyHelperTTL(): number {
  const envTtl = process.env.COURSE_CODE_API_KEY_HELPER_TTL_MS
  ...
  if (envTtl) {
    ...
    logForDebugging(
      `Found COURSE_CODE_API_KEY_HELPER_TTL_MS env var, but it was not a valid number. Got ${envTtl}`,
      { level: 'error' },
    )
  }
...
async function _executeApiKeyHelper(
  ...
  // Parse the command string into executable + args to avoid shell injection.
  // If parsing fails (malformed shell syntax), fall back to shell execution
  // with a timeout — legitimate credential helpers may use shell features.
  const parsed = tryParseShellCommand(apiKeyHelper)
  const useArrayForm = parsed.success && !hasMalformedTokens(apiKeyHelper, parsed.tokens)
  const result = useArrayForm
    ? await execa(
        String(parsed.tokens[0]),
        parsed.tokens.slice(1).map(t => (typeof t === 'string' ? t : String(t))),
        { timeout: 10 * 60 * 1000, reject: false },
      )
    : await execa(apiKeyHelper, {
        shell: true,
        timeout: 10 * 60 * 1000,
        reject: false,
      })
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running awsAuthRefresh (in settings or ~/.course.json):',
            )
...
async function getAwsCredsFromCredentialExport(): Promise<{
  ...
  try {
    ...
    try {
      ...
      const parsedAws = tryParseShellCommand(awsCredentialExport)
      const useArrayFormAws = parsedAws.success && !hasMalformedTokens(awsCredentialExport, parsedAws.tokens)
      const result = useArrayFormAws
        ? await execa(
            String(parsedAws.tokens[0]),
            parsedAws.tokens.slice(1).map(t => (typeof t === 'string' ? t : String(t))),
            { reject: false },
          )
        : await execa(awsCredentialExport, {
            shell: true,
            reject: false,
          })
      ...
      const message = chalk.red(
        'Error getting AWS credentials from awsCredentialExport (in settings or ~/.course.json):',
      )
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running gcpAuthRefresh (in settings or ~/.course.json):',
            )
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(message)
        authStatusManager.endAuthentication(false)
...
export const getClaudeAIOAuthTokens = memoize((): OAuthTokens | null => {
  ...
  if (process.env.COURSE_CODE_OAUTH_TOKEN) {
    ...
    return {
      accessToken: process.env.COURSE_CODE_OAUTH_TOKEN,
      refreshToken: null,
      expiresAt: null,
...
export function getOtelHeadersFromHelper(): Record<string, string> {
  ...
  const debounceMs = parseInt(
    process.env.COURSE_CODE_OTEL_HEADERS_HELPER_DEBOUNCE_MS ||
...
export function getAccountInformation() {
  ...
  if (
    authTokenSource === 'COURSE_CODE_OAUTH_TOKEN' ||
    authTokenSource === 'COURSE_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ) {
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  // in ~/.course.json is user-writable and cannot be trusted.
  const { source } = getAuthTokenSource()
  const isEnvVarToken =
    source === 'COURSE_CODE_OAUTH_TOKEN' ||
    source === 'COURSE_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ...
  if (!profile) {
    ...
    return {
      ...
      message:
        ...
        `verification (tokens from 'course setup-token' do not include this scope).\n` +
        `Try again, or obtain a full-scope token via 'course auth login'.`,
    }
  }
  ...
  if (isEnvVarToken) {
    const envVarName =
      source === 'COURSE_CODE_OAUTH_TOKEN'
        ...
        : 'COURSE_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR'
    return {
      valid: false,
      message:
  ...
  return {
    ...
    message:
      ...
      `Please log in with the correct organization: course auth login`,
  }
}
