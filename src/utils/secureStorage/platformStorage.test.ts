...
describe("Secure Storage Platform Implementations", () => {
  ...
  describe("Config-Dir Isolation", () => {
    ...
    test("service name changes with COURSE_CONFIG_DIR", () => {
      ...
      process.env.COURSE_CONFIG_DIR = "/tmp/preferred-config";
      delete process.env.COURSE_CONFIG_DIR;
    ...
    test("Linux storage uses scoped service name", () => {
      delete process.env.COURSE_CONFIG_DIR;
      delete process.env.COURSE_CONFIG_DIR;
      process.env.COURSE_CONFIG_DIR = "/tmp/linux-scoped";
    ...
    test("Linux storage uses COURSE_CONFIG_DIR scoped service name", () => {
      process.env.COURSE_CONFIG_DIR = "/tmp/linux-preferred-scoped";
      delete process.env.COURSE_CONFIG_DIR;
      const expectedName = getSecureStorageServiceName(CREDENTIALS_SERVICE_SUFFIX);
    ...
    test("Windows storage uses scoped resource name", () => {
      delete process.env.COURSE_CONFIG_DIR;
      delete process.env.COURSE_CONFIG_DIR;
      process.env.COURSE_CONFIG_DIR = "/tmp/win-scoped";
      const expectedName = getSecureStorageServiceName(CREDENTIALS_SERVICE_SUFFIX);
    ...
    test("Windows storage uses COURSE_CONFIG_DIR scoped resource name", () => {
      process.env.COURSE_CONFIG_DIR = "/tmp/win-preferred-scoped";
      delete process.env.COURSE_CONFIG_DIR;
      const expectedName = getSecureStorageServiceName(CREDENTIALS_SERVICE_SUFFIX);
  ...
  describe("Windows PowerShell Escaping", () => {
    ...
    test("delete() includes legacy assembly load when explicitly enabled", () => {
      process.env.COURSE_ENABLE_LEGACY_WINDOWS_PASSWORDVAULT = "1";
      windowsCredentialStorage.delete();
