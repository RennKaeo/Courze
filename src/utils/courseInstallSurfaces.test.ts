...
beforeEach(async () => {
  await acquireSharedMutationLock('utils/courseInstallSurfaces.test.ts')
})
...
test('install command displays ~/.local/bin/course on non-Windows', async () => {
  await mockEnvPlatform('darwin')

  const { getInstallationPath } = await importFreshInstallCommand()

  expect(getInstallationPath()).toBe('~/.local/bin/course')
})

test('install command displays course.exe path on Windows', async () => {
  await mockEnvPlatform('win32')

  const { getInstallationPath } = await importFreshInstallCommand()

  expect(getInstallationPath()).toBe(
    join(homedir(), '.local', 'bin', 'course.exe').replace(/\//g, '\\'),
  )
})

test('native installer uses course launcher for Course Code package', async () => {
  ...
  expect(getExecutableName('linux-x64')).toBe('course')
  expect(getExecutableName('win32-x64')).toBe('course.exe')
})
...
test('deep-link protocol resolver uses course launcher for Course Code package', async () => {
  ;(globalThis as Record<string, unknown>).MACRO = {
    PACKAGE_URL: '@renskaeo/courze',
  }
  ...
  expect(getProtocolBinaryName('linux')).toBe('course')
  expect(getProtocolBinaryName('win32')).toBe('course.exe')
})
    ...
    void render(
      createElement(Install, {
        ...
        onDone: (result: string) => {
          try {
            expect(result).toBe('Course Code installation completed successfully')
            resolve()
          } catch (error) {
            reject(error)
          }
...
test('cleanupNpmInstallations removes both course and legacy claude local install dirs', async () => {
  ...
  mock.module('./envUtils.js', () => ({
    ...realEnvUtils,
    getClaudeConfigHomeDir: () => join(homedir(), '.course'),
  }))

  const { cleanupNpmInstallations } = await importFreshInstaller()
  await cleanupNpmInstallations()

  expect(removedPaths).toContain(join(homedir(), '.course', 'local'))
  expect(removedPaths).toContain(join(homedir(), '.claude', 'local'))
})

test('cleanupNpmInstallations manual fallback removes course npm shim', async () => {
  await mockEnvPlatform('darwin')

  const testHome = join(process.cwd(), 'work', 'course-install-home-test')
  const npmPrefix = join(testHome, '.npm-global')
  const shimPath = join(npmPrefix, 'bin', 'course')
  ;(globalThis as Record<string, unknown>).MACRO = {
    PACKAGE_URL: '@renskaeo/courze',
  }
