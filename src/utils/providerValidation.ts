...
function getOpenAIMissingKeyMessage(): string {
  ...
  return [
    ...
    `To recover, run /provider and switch provider, or set COURSE_CODE_USE_OPENAI=0 in your shell environment.`,
...
function getRuntimeValidationTarget(
  ...
  const useOpenAI = isEnvTruthy(env.COURSE_CODE_USE_OPENAI)
...
export async function validateProviderEnvForStartupOrExit(
  ...
  console.error(
    `Warning: provider configuration is incomplete.\n${error}\nCourse Code will continue starting so you can run /provider and repair the saved provider settings.`,
  )
}
