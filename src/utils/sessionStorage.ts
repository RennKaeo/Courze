...
function getEntrypoint(): string | undefined {
  return process.env.COURSE_CODE_ENTRYPOINT
}
 ...
 * This ensures tests with different COURSE_CONFIG_DIR values
...
class Project {
   ...
   * COURSE_CODE_SKIP_PROMPT_HISTORY should suppress all transcript writes.
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    if (!isEnvTruthy(process.env.COURSE_CODE_DISABLE_PRECOMPACT_SKIP)) {
    ...
    // COURSE_CODE_DISABLE_PRECOMPACT_SKIP is set (that kill switch means
    ...
    if (
      !opts?.keepAllLeaves &&
      !hasPreservedSegment &&
      !isEnvTruthy(process.env.COURSE_CODE_DISABLE_PRECOMPACT_SKIP) &&
...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    if (
      ...
      isEnvTruthy(process.env.COURSE_CODE_SAVE_HOOK_ADDITIONAL_CONTEXT)
    ) {
      return true
    }
    return false
