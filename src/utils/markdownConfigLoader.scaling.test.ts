...
// over COURSE_CODE_MAX_MARKDOWN_FILE_SIZE_BYTES (default 256 KiB) so that
...
const SAVED_ENV = {
  ...
  COURSE_CODE_MAX_MARKDOWN_FILE_SIZE_BYTES:
    process.env.COURSE_CODE_MAX_MARKDOWN_FILE_SIZE_BYTES,
}
...
beforeEach(async () => {
  ...
  tempDir = await mkdtemp(join(tmpdir(), 'course-md-scaling-'))
  process.env.COURSE_CONFIG_DIR = join(tempDir, '.course')
  process.env.COURSE_CODE_USE_NATIVE_FILE_SEARCH = '1'
  delete process.env.COURSE_CODE_MAX_MARKDOWN_FILE_SIZE_BYTES
...
afterEach(async () => {
  try {
    ...
    restore('COURSE_CONFIG_DIR')
    restore('COURSE_CODE_USE_NATIVE_FILE_SEARCH')
    restore('COURSE_CODE_MAX_MARKDOWN_FILE_SIZE_BYTES')
    loadMarkdownFilesForSubdir.cache.clear?.()
    clearOversizedMarkdownSkipsForTesting()
  } finally {
    releaseSharedMutationLock()
...
describe('loadMarkdownFilesForSubdir (#769 scaling)', () => {
  test('loads >batch-size agent files without losing any', async () => {
    const agentsDir = join(process.env.COURSE_CONFIG_DIR!, 'agents')
  ...
  test('skips files exceeding the max size and keeps small siblings', async () => {
    const agentsDir = join(process.env.COURSE_CONFIG_DIR!, 'agents')
    process.env.COURSE_CODE_MAX_MARKDOWN_FILE_SIZE_BYTES = '1024'
    loadMarkdownFilesForSubdir.cache.clear?.()
  ...
  test('size cap is overridable via env var', async () => {
    const agentsDir = join(process.env.COURSE_CONFIG_DIR!, 'agents')
    process.env.COURSE_CODE_MAX_MARKDOWN_FILE_SIZE_BYTES = '65536'
    loadMarkdownFilesForSubdir.cache.clear?.()
