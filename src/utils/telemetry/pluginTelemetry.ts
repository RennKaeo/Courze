 ...
 * managedNames=null — the session-level courze_plugin_enabled_for_session
...
/**
 * Emit courze_plugin_enabled_for_session once per enabled plugin at session
 * start. Supplements courze_skill_loaded (which still fires per-skill) — use
...
export function logPluginsEnabledForSession(
  ...
  for (const plugin of plugins) {
    ...
    logEvent('courze_plugin_enabled_for_session', {
      _PROTO_plugin_name:
...
/**
 * Emit courze_plugin_load_failed once per error surfaced by session-start
 * plugin loading. Pairs with courze_plugin_enabled_for_session so dashboards
...
export function logPluginLoadErrors(
  ...
  for (const err of errors) {
    ...
    logEvent('courze_plugin_load_failed', {
      error_category:
