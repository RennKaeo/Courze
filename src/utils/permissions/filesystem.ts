 ...
 * .course/skills/{name}/ directory (global), plus the legacy global
...
export function getClaudeSkillScope(
  ...
  const bases = [
    ...
    {
      dir: expandPath(join(homedir(), '.course', 'skills')),
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    normalizedPath.endsWith(`${sep}.course${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.course${sep}settings.local.json`) ||
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const openCommandsDir = join(getOriginalCwd(), '.course', 'commands')
  const openAgentsDir = join(getOriginalCwd(), '.course', 'agents')
  const openSkillsDir = join(getOriginalCwd(), '.course', 'skills')
 ...
 * Controlled by the courze_scratch Statsig gate.
 */
export function isScratchpadEnabled(): boolean {
  return checkStatsigFeatureGate_CACHED_MAY_BE_STALE('courze_scratch')
}
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (claudeFolderAllowRule) {
    ...
    // '~/.course/**', and legacy '~/.claude/**') plus narrowed skill
    // patterns like '~/.course/skills/my-skill/**' so users can grant
...
export function checkEditableInternalPath(
  ...
  if (
    ...
    return {
      ...
      decisionReason: {
        type: 'other',
        reason: 'Course Code commit message file is allowed for writing',
      },
    }
  }
