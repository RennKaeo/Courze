...
if (isBeingDebugged()) {
  ...
  // eslint-disable-next-line custom-rules/no-top-level-side-effects
  process.stderr.write('[course] Debugger detected. Attaching a debugger is not supported in external builds. Exiting.\n')
  process.exit(1);
}
...
function getCertEnvVarTelemetry(): Record<string, boolean> {
  ...
  if (process.env.COURSE_CODE_CLIENT_CERT) {
...
export function startDeferredPrefetches(): void {
  ...
  if (isEnvTruthy(process.env.COURSE_CODE_EXIT_AFTER_FIRST_RENDER) ||
  ...
  if (isEnvTruthy(process.env.COURSE_CODE_USE_BEDROCK) && !isEnvTruthy(process.env.COURSE_CODE_SKIP_BEDROCK_AUTH)) {
    void prefetchAwsCredentialsAndBedRockInfoIfSafe();
  }
  if (isEnvTruthy(process.env.COURSE_CODE_USE_VERTEX) && !isEnvTruthy(process.env.COURSE_CODE_SKIP_VERTEX_AUTH)) {
...
function initializeEntrypoint(isNonInteractive: boolean): void {
  ...
  if (process.env.COURSE_CODE_ENTRYPOINT) {
    return;
  }
  ...
  if (mcpIndex !== -1 && cliArgs[mcpIndex + 1] === 'serve') {
    process.env.COURSE_CODE_ENTRYPOINT = 'mcp';
    return;
  }
  if (isEnvTruthy(process.env.COURSE_CODE_ACTION)) {
    process.env.COURSE_CODE_ENTRYPOINT = 'course-code-github-action';
    return;
  }

  // Note: 'local-agent' entrypoint is set by the local agent mode launcher
  // via COURSE_CODE_ENTRYPOINT env var (handled by early return above)

  // Set based on interactive status
  process.env.COURSE_CODE_ENTRYPOINT = isNonInteractive ? 'sdk-cli' : 'cli';
}
...
export async function main() {
  ...
  if (feature('LODESTONE')) {
    ...
    if (process.platform === 'darwin' && process.env.__CFBundleIdentifier === 'com.anthropic.course-code-url-handler') {
      const {
        enableConfigs
      } = await import('./utils/config.js');
      enableConfigs();
  ...
  if (feature('SSH_REMOTE') && _pendingSSH) {
    ...
    if (rawCliArgs[0] === 'ssh' && rawCliArgs[1] && !rawCliArgs[1].startsWith('-')) {
      ...
      if (rest.includes('-p') || rest.includes('--print')) {
        process.stderr.write('Error: headless (-p/--print) mode is not supported with course ssh\n');
  ...
  const clientType = (() => {
    ...
    if (process.env.COURSE_CODE_ENTRYPOINT === 'sdk-ts') return 'sdk-typescript';
    if (process.env.COURSE_CODE_ENTRYPOINT === 'sdk-py') return 'sdk-python';
    if (process.env.COURSE_CODE_ENTRYPOINT === 'sdk-cli') return 'sdk-cli';
    if (process.env.COURSE_CODE_ENTRYPOINT === 'claude-vscode') return 'claude-vscode';
    if (process.env.COURSE_CODE_ENTRYPOINT === 'local-agent') return 'local-agent';
    if (process.env.COURSE_CODE_ENTRYPOINT === 'claude-desktop') return 'claude-desktop';

    // Check if session-ingress token is provided (indicates remote session)
    const hasSessionIngressToken = process.env.COURSE_CODE_SESSION_ACCESS_TOKEN || process.env.COURSE_CODE_WEBSOCKET_AUTH_FILE_DESCRIPTOR;
    if (process.env.COURSE_CODE_ENTRYPOINT === 'remote' || hasSessionIngressToken) {
      return 'remote';
    }
    return 'cli';
  })();
  setClientType(clientType);
  const previewFormat = process.env.COURSE_CODE_QUESTION_PREVIEW_FORMAT;
  ...
  if (process.env.COURSE_CODE_ENVIRONMENT_KIND === 'bridge') {
...
async function run(): Promise<CommanderCommand> {
  ...
  program.hook('preAction', async thisCommand => {
    ...
    if (!isEnvTruthy(process.env.COURSE_CODE_DISABLE_TERMINAL_TITLE)) {
  ...
  program.name('course').description(`Course Code - starts an interactive session by default, use -p/--print for non-interactive output`).argument('[prompt]', 'Your prompt', String)
  ...
  .option('--provider-env-file <path>', 'Load provider environment variables from a file before validation (repeatable; existing values win)', (val: string, prev: string[]) => [...prev, val], [] as string[]).action(async (prompt, options) => {
    ...
    if (feature('BG_SESSIONS') && agentCli) {
      process.env.COURSE_CODE_AGENT = agentCli;
    }
    ...
    if (isAgentSwarmsEnabled()) {
      ...
      // These replace the COURSE_CODE_* environment variables
    ...
    const effectiveIncludePartialMessages = includePartialMessages || isEnvTruthy(process.env.COURSE_CODE_INCLUDE_PARTIAL_MESSAGES);

    // Enable all hook event types when explicitly requested via SDK option
    // or when running in COURSE_CODE_REMOTE mode (CCR needs them).
    // Without this, only SessionStart and Setup events are emitted.
    if (includeHookEvents || isEnvTruthy(process.env.COURSE_CODE_REMOTE)) {
    ...
    if (fileSpecs && fileSpecs.length > 0) {
      // Get session ingress token (provided by EnvManager via COURSE_CODE_SESSION_ACCESS_TOKEN)
      const sessionToken = getSessionIngressAuthToken();
      if (!sessionToken) {
        process.stderr.write(chalk.red('Error: Session token required for file downloads. COURSE_CODE_SESSION_ACCESS_TOKEN must be set.\n'));
        process.exit(1);
      }

      // Resolve session ID: prefer remote session ID, fall back to internal session ID
      const fileSessionId = process.env.COURSE_CODE_REMOTE_SESSION_ID || getSessionId();
    ...
    if (feature('COORDINATOR_MODE') && isEnvTruthy(process.env.COURSE_CODE_COORDINATOR_MODE)) {
      const {
    ...
    if (process.env.COURSE_CODE_ENTRYPOINT !== 'local-agent') {
      initBuiltinPlugins();
      initBundledSkills();
    }
    ...
    if ((feature('PROACTIVE') || feature('KAIROS')) && ((options as {
      proactive?: boolean;
    }).proactive || isEnvTruthy(process.env.COURSE_CODE_PROACTIVE)) && !coordinatorModeModule?.isCoordinatorMode()) {
      /* eslint-disable @typescript-eslint/no-require-imports */
    ...
    if (!isNonInteractiveSession) {
      ...
      if (
        ...
        !isEnvTruthy(process.env.COURSE_CODE_USE_OPENAI) &&
    ...
    if (options.continue) {
      ...
      if (!targetSessionId) {
        ...
        if (sessions.length === 0) {
          ...
          return await exitWithMessage(root, `Assistant installed in ${installedDir}. The daemon is starting up — run \`course assistant\` again in a few seconds to connect.`, {
            exitCode: 0,
      ...
      if (remote !== null) {
        ...
        if (!isRemoteTuiEnabled) {
          ...
          process.stdout.write(`Resume with: course --teleport ${createdSession.id}\n`);
          await gracefulShutdown(0);
          process.exit(0);
        }
            ...
            if (repoValidation.status === 'mismatch' || repoValidation.status === 'not_in_repo') {
              ...
              if (sessionRepo) {
                ...
                if (existingPaths.length > 0) {
                  ...
                  throw new TeleportOperationError(`You must run course --teleport ${teleport} from a checkout of ${sessionRepo}.`, chalk.red(`You must run course --teleport ${teleport} from a checkout of ${chalk.bold(sessionRepo)}.\n`));
                }
              }
  ...
  }).version(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (Course Code)`, '-v, --version', 'Output the version number');
  ...
  mcp.command('serve').description(`Start the Course Code MCP server`).option('-d, --debug', 'Enable debug mode', () => true).option('--verbose', 'Override verbose mode setting from config', () => true).action(async ({
    debug,
    verbose
  }: {
    debug?: boolean;
  ...
  if (feature('DIRECT_CONNECT')) {
    program.command('server').description('Start an Course Code session server').option('--port <number>', 'HTTP port', '0').option('--host <string>', 'Bind address', '0.0.0.0').option('--auth-token <token>', 'Bearer token for auth').option('--unix <path>', 'Listen on a unix domain socket').option('--workspace <dir>', 'Default working directory for sessions that do not specify cwd').option('--idle-timeout <ms>', 'Idle timeout for detached sessions in ms (0 = never expire)', '600000').option('--max-sessions <n>', 'Maximum concurrent sessions (0 = unlimited)', '32').action(async (opts: {
      ...
      if (existing) {
        process.stderr.write(`An Course Code server is already running (pid ${existing.pid}) at ${existing.httpUrl}\n`);
        process.exit(1);
      }
  ...
  if (feature('DIRECT_CONNECT')) {
    program.command('open <cc-url>').description('Connect to an Course Code server (internal — use cc:// URLs)').option('-p, --print [prompt]', 'Print mode (headless)').option('--output-format <format>', 'Output format: text, json, stream-json', 'text').action(async (ccUrl: string, opts: {
  ...
  const pluginCmd = program.command('plugin').alias('plugins').description('Manage Course Code plugins').configureHelp(createSortedHelpConfig());
  ...
  const marketplaceCmd = pluginCmd.command('marketplace').description('Manage Course Code marketplaces').configureHelp(createSortedHelpConfig());
  ...
  if (feature('KAIROS')) {
    program.command('assistant [sessionId]').description('Attach the REPL as a client to a running bridge session. Discovers sessions via API if no sessionId given.').action(() => {
      ...
      process.stderr.write('Usage: course assistant [sessionId]\n\n' + 'Attach the REPL as a viewer client to a running bridge session.\n' + 'Omit sessionId to discover and pick from available sessions.\n');
      process.exit(1);
    });
  }
  ...
  const doctorCommand = program
    .command('doctor')
    .description('Check the health of your Course Code auto-updater. Note: The workspace trust dialog is skipped and stdio servers from .mcp.json are spawned for health checks. Only use this command in directories you trust.');
  doctorCommand
    .command('report')
  ...
  // claude install
  program.command('install [target]').description('Install Course Code native build. Use [target] to specify version (stable, latest, or specific version)').option('--force', 'Force installation even if already installed').action(async (target: string | undefined, options: {
    force?: boolean;
  }) => {
    const {
      installHandler
...
function maybeActivateProactive(options: unknown): void {
  if ((feature('PROACTIVE') || feature('KAIROS')) && ((options as {
    proactive?: boolean;
  }).proactive || isEnvTruthy(process.env.COURSE_CODE_PROACTIVE))) {
...
function maybeActivateBrief(options: unknown): void {
  ...
  const briefEnv = isEnvTruthy(process.env.COURSE_CODE_BRIEF);
  if (!briefFlag && !briefEnv) return;
  // --brief / COURSE_CODE_BRIEF are explicit opt-ins: check entitlement,
  ...
  // COURSE_CODE_BRIEF=1 alone force-enables for dev/testing — no GB gate
