...
export async function resolveAttachments(
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    // REPL does, at main.tsx init). COURSE_CODE_BRIEF_UPLOAD lets a host that
    // runs the CLI as a subprocess opt in — e.g. the cowork desktop bridge,
    // which already passes COURSE_CODE_OAUTH_TOKEN for auth.
    const shouldUpload =
      uploadCtx.replBridgeEnabled ||
      isEnvTruthy(process.env.COURSE_CODE_BRIEF_UPLOAD)
