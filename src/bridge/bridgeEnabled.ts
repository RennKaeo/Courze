 ...
 * (setup-token, COURSE_CODE_OAUTH_TOKEN env var, or pre-scope-expansion
...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isClaudeAISubscriber()) {
      return 'Remote Control requires a claude.ai subscription. Run `course auth login` to sign in with your claude.ai account.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires a full-scope login token. Long-lived tokens (from `course setup-token` or COURSE_CODE_OAUTH_TOKEN) are limited to inference-only for security reasons. Run `course auth login` to use Remote Control.'
    }
