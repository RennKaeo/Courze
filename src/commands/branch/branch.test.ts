...
async function setupSourceTranscript(
  ...
  const projectCwd = await mkdtemp(join(tmpdir(), 'course-branch-cwd-'))
  const configDir = await mkdtemp(join(tmpdir(), 'course-branch-config-'))
  const sessionProjectDir = options.separateSessionProjectDir
    ? await mkdtemp(join(tmpdir(), 'course-branch-session-dir-'))
    : null
