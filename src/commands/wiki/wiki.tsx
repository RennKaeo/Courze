...
Manage the Course Code project wiki stored in .course/wiki.
...
function formatInitResult(result: Awaited<ReturnType<typeof initializeWiki>>): string {
  const lines = [`Initialized Course Code wiki at ${result.root}`]
...
function formatStatus(status: Awaited<ReturnType<typeof getWikiStatus>>): string {
  if (!status.initialized) {
    return `Course Code wiki is not initialized in this project.\n\nRun /wiki init to create ${status.root}.`
  }

  const lines = [
    'Course Code wiki status',
    '',
    `Root: ${status.root}`,
...
function formatIngestResult(
  ...
  return [
    `Ingested ${result.sourceFile} into the Course Code wiki.`,
    '',
    `Title: ${result.title}`,
