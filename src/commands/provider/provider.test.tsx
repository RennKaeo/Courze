...
const ORIGINAL_SIMPLE_ENV = process.env.COURSE_CODE_SIMPLE
...
afterEach(() => {
  try {
    ...
    if (ORIGINAL_SIMPLE_ENV === undefined) {
      ...
      process.env.COURSE_CODE_SIMPLE = ORIGINAL_SIMPLE_ENV
    }
...
test('buildProfileSaveMessage maps provider fields without echoing secrets', () => {
  const message = buildProfileSaveMessage(
    ...
    'D:/codings/Opensource/course/.courzerc.json',
  )
...
test('buildProfileSaveMessage reflects immediate Codex activation for existing credentials', () => {
  ...
  expect(message).toContain('Course Code switched to it for this session.')
  expect(message).not.toContain('Restart Course Code to use it.')
})
...
test('explicitly declared env takes precedence over applySavedProfileToCurrentSession', async () => {
  ...
  const processEnv: NodeJS.ProcessEnv = {
    ...
    COURSE_CODE_PROVIDER_PROFILE_ENV_APPLIED_ID: 'provider_old',
  }
  ...
  expect(processEnv.COURSE_CODE_USE_OPENAI).toBe('1')
  ...
  expect(processEnv.COURSE_CODE_PROVIDER_PROFILE_ENV_APPLIED).toBeUndefined()
  expect(processEnv.COURSE_CODE_PROVIDER_PROFILE_ENV_APPLIED_ID).toBeUndefined()
})
