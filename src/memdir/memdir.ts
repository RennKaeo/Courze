...
function logMemoryDirCounts(
  ...
  void fs.readdir(memoryDir).then(
    ...
    () => {
      ...
      logEvent('courze_memdir_loaded', baseMetadata)
    },
  )
}
...
export function buildSearchingPastContextSection(autoMemDir: string): string[] {
  if (!getFeatureValue_CACHED_MAY_BE_STALE('courze_coral_fern', false)) {
    return []
  }
...
export async function loadMemoryPrompt(): Promise<string | null> {
  ...
  // means the !autoEnabled case falls through to the courze_memdir_disabled
  ...
  logEvent('courze_memdir_disabled', {
    disabled_by_env_var: isEnvTruthy(
      process.env.COURSE_CODE_DISABLE_AUTO_MEMORY,
    ),
    disabled_by_setting:
      !isEnvTruthy(process.env.COURSE_CODE_DISABLE_AUTO_MEMORY) &&
  ...
  if (getFeatureValue_CACHED_MAY_BE_STALE('courze_herring_clock', false)) {
    logEvent('courze_team_memdir_disabled', {})
  }
  return null
}
