...
export function getLocalISODate(): string {
  ...
  if (process.env.COURSE_CODE_OVERRIDE_DATE) {
    return process.env.COURSE_CODE_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.COURSE_CODE_OVERRIDE_DATE
    ? new Date(process.env.COURSE_CODE_OVERRIDE_DATE)
    : new Date()
