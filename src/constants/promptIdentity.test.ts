...
beforeAll(async () => {
  ...
  ;(globalThis as Record<string, unknown>).MACRO = {
    ...
    ISSUES_EXPLAINER:
      'report the issue at https://github.com/RennKaeo/Courze/issues',
...
test('CLI identity prefixes describe Course Code instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('Course Code')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('Course Code')
...
test('simple mode identity describes Course Code instead of Claude Code', async () => {
  ...
  expect(prompt[0]).toContain('Course Code')
...
test('built-in agent prompts describe Course Code instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('Course Code')
  ...
  expect(generalPrompt).toContain('Course Code')
  ...
  expect(explorePrompt).toContain('Course Code')
  ...
  expect(planPrompt).toContain('Course Code')
  ...
  expect(guidePrompt).toContain('Course Code')
  expect(guidePrompt).toContain('You are the Course Code guide agent.')
  expect(guidePrompt).toContain('**Course Code** (the CLI tool)')
