...
export function isCoordinatorMode(): boolean {
  if (feature('COORDINATOR_MODE')) {
    return isEnvTruthy(process.env.COURSE_CODE_COORDINATOR_MODE)
  }
  return false
}
...
export function matchSessionMode(
  ...
  if (sessionIsCoordinator) {
    process.env.COURSE_CODE_COORDINATOR_MODE = '1'
  } else {
    delete process.env.COURSE_CODE_COORDINATOR_MODE
  }

  logEvent('courze_coordinator_mode_switched', {
...
export function getCoordinatorUserContext(
  ...
  const workerTools = isEnvTruthy(process.env.COURSE_CODE_SIMPLE)
...
export function getCoordinatorSystemPrompt(): string {
  const workerCapabilities = isEnvTruthy(process.env.COURSE_CODE_SIMPLE)
...
2. Precise git operation: "Create a new branch from main called 'fix/session-expiry'. Cherry-pick only commit abc123 onto it. Push and create a draft PR targeting main. Add anthropics/course-code as reviewer. Report the PR URL."
