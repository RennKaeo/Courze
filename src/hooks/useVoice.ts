...
export function useVoice({
  ...
  // Tracked for the courze_voice_recording_completed analytics event.
    ...
    void finalizePromise
      .then(async finalizeSource => {
        ...
        if (
          ...
          logEvent('courze_voice_silent_drop_replay', {
            recordingDurationMs,
        ...
        logEvent('courze_voice_recording_completed', {
        ...
        if (text) {
          ...
          if (!wsConnected) {
            ...
            onErrorRef.current?.(
              'No audio detected from microphone. Check that the correct input device is selected and that Course Code has microphone access.',
            )
          } else {
  ...
  async function startRecordingSession(): Promise<void> {
    ...
    logEvent('courze_voice_recording_started', {
          ...
          onError: (error: string, opts?: { fatal?: boolean }) => {
            ...
            if (
              ...
              if (!retryUsedRef.current) {
                ...
                logEvent('courze_voice_stream_early_retry', {})
                connectionRef.current = null
                attemptGenRef.current++
                setTimeout(
