...
/**
 * Surfaces plugin-install prompts driven by `<course-code-hint />` tags
 * that CLIs/SDKs emit to stderr. See docs/course-code-hints.md.
 *
...
import { clearPendingHint, getPendingHintSnapshot, markShownThisSession, subscribeToPendingHint } from '../utils/courseCodeHints.js';
...
type UseCourseCodeHintRecommendationResult = {
  recommendation: PluginHintRecommendation | null;
  handleResponse: (response: 'yes' | 'no' | 'disable') => void;
};
export function useCourseCodeHintRecommendation() {
  const $ = _c(11);
  ...
  if ($[0] !== pendingHint || $[1] !== tryResolve) {
    t0 = () => {
      ...
      tryResolve(async () => {
        ...
        if (resolved) {
          logForDebugging(`[useCourseCodeHintRecommendation] surfacing ${resolved.pluginId} from ${resolved.sourceCommand}`);
  ...
  if ($[4] !== addNotification || $[5] !== clearRecommendation || $[6] !== recommendation) {
    t2 = response => {
      ...
      logEvent("courze_plugin_hint_response", {
