...
export async function authLogin({
  ...
  const envRefreshToken = process.env.COURSE_CODE_OAUTH_REFRESH_TOKEN
  if (envRefreshToken) {
    const envScopes = process.env.COURSE_CODE_OAUTH_SCOPES
    if (!envScopes) {
      process.stderr.write(
        'COURSE_CODE_OAUTH_SCOPES is required when using COURSE_CODE_OAUTH_REFRESH_TOKEN.\n' +
  ...
  if (opts.text) {
    ...
    if (!loggedIn) {
      process.stdout.write(
        'Not logged in. Run course auth login to authenticate.\n',
      )
    }
  } else {
