...
export async function pluginListHandler(options: {
  ...
  if (options.cowork) setUseCoworkPlugins(true)
  logEvent('courze_plugin_list_command', {})
  ...
  if (pluginIds.length === 0 && inlinePlugins.length === 0) {
    ...
    if (inlineLoadErrors.length === 0) {
      cliOk(
        'No plugins installed. Use `course plugin install` to install a plugin.',
      )
    }
  }
...
export async function marketplaceAddHandler(
  ...
  try {
    ...
    logEvent('courze_marketplace_added', {
      source_type:
...
export async function marketplaceRemoveHandler(
  ...
  try {
    ...
    logEvent('courze_marketplace_removed', {
      marketplace_name:
...
export async function marketplaceUpdateHandler(
  ...
  try {
    if (name) {
      ...
      logEvent('courze_marketplace_updated', {
        marketplace_name:
          name as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
      })
      ...
      logEvent('courze_marketplace_updated_all', {
        count:
...
export async function pluginInstallHandler(
  ...
  logEvent('courze_plugin_install_command', {
...
export async function pluginUninstallHandler(
  ...
  const { name, marketplace } = parsePluginIdentifier(plugin)
  logEvent('courze_plugin_uninstall_command', {
    _PROTO_plugin_name: name as AnalyticsMetadata_I_VERIFIED_THIS_IS_PII_TAGGED,
    ...(marketplace && {
      _PROTO_marketplace_name:
        marketplace as AnalyticsMetadata_I_VERIFIED_THIS_IS_PII_TAGGED,
...
export async function pluginEnableHandler(
  ...
  const { name, marketplace } = parsePluginIdentifier(plugin)
  logEvent('courze_plugin_enable_command', {
    _PROTO_plugin_name: name as AnalyticsMetadata_I_VERIFIED_THIS_IS_PII_TAGGED,
    ...(marketplace && {
      _PROTO_marketplace_name:
        marketplace as AnalyticsMetadata_I_VERIFIED_THIS_IS_PII_TAGGED,
...
export async function pluginDisableHandler(
  ...
  if (options.all) {
    ...
    logEvent('courze_plugin_disable_command', {})
...
export async function pluginUpdateHandler(
  ...
  if (options.cowork) setUseCoworkPlugins(true)
  const { name, marketplace } = parsePluginIdentifier(plugin)
  logEvent('courze_plugin_update_command', {
    _PROTO_plugin_name: name as AnalyticsMetadata_I_VERIFIED_THIS_IS_PII_TAGGED,
    ...(marketplace && {
      _PROTO_marketplace_name:
        marketplace as AnalyticsMetadata_I_VERIFIED_THIS_IS_PII_TAGGED,
