...
| \`.course/settings.json\` | Project | Commit | Team-wide hooks, permissions, plugins |
| \`.course/settings.local.json\` | Project | Gitignore | Personal overrides for this project |
...
{
  "permissions": {
    "allow": ["Bash(npm:*)", "Edit(.course)", "Read"],
    "deny": ["Bash(rm -rf:*)"],
    "ask": ["Write(/etc/*)"],
...
\`\`\`
Plugin syntax: \`plugin-name@source\` where source is \`course-code-marketplace\`, \`claude-plugins-official\`, or \`builtin\`.
...
4. **Write the JSON.** Merge into the target file (schema shape in the "Hook Structure" section above). If this creates \`.course/settings.local.json\` for the first time, add it to .gitignore — the Write tool doesn't auto-gitignore it.
...
2. **Read**: \`.course/settings.json\` (or create if missing)
...
1. **Check the settings file** - Read ${USER_SETTINGS_PATH_TOKEN} or .course/settings.json
2. **Verify JSON syntax** - Invalid JSON silently fails
3. **Check the matcher** - Does it match the tool name? (e.g., "Bash", "Write", "Edit")
4. **Check hook type** - Is it "command", "prompt", or "agent"?
5. **Test the command** - Run the hook command manually to see if it works
6. **Use --debug** - Run \`course --debug\` to see hook execution logs
`
