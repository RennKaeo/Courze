...
function XaiOAuthSetup({
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text>
        Sign in with your xAI account in the browser. Course Code will store
      ...
      <Text dimColor>
        ...
        expected. Course Code uses xAI's shared OAuth client.
      </Text>
...
function CodexOAuthSetup({
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text>
        Sign in with your ChatGPT account in the browser. Course Code will store
...
export function ProviderManager({ mode, onDone }: Props): React.ReactNode {
  ...
  const initialIsGithubActive = isEnvTruthy(process.env.COURSE_CODE_USE_GITHUB)
  ...
  const refreshGithubProviderState = React.useCallback((): void => {
    ...
    const githubActive = isEnvTruthy(process.env.COURSE_CODE_USE_GITHUB)
    ...
    void (async () => {
      ...
      setIsGithubActive(isEnvTruthy(process.env.COURSE_CODE_USE_GITHUB))
      setIsGithubCredentialSourceResolved(true)
    })()
  }, [])
  ...
  function buildCodexOAuthActivationMessage(options: {
    ...
    if (options.warnings.length > 0) {
      return `${options.prefix}. Course Code switched to it for this session with warnings: ${formatWarningsForMessage(options.warnings)}`
    }

    return `${options.prefix}. Course Code switched to it for this session.`
  }
  ...
  function buildXaiOAuthActivationMessage(options: {
    ...
    if (options.warnings.length > 0) {
      return `${options.prefix}. Course Code switched to it for this session with warnings: ${options.warnings.join('; ')}.`
    }
    return `${options.prefix}. Course Code switched to it for this session.`
  }
  ...
  function activateGithubProvider(): string | null {
    const { error } = updateSettingsForSource('userSettings', {
      env: {
        ...
        COURSE_CODE_USE_OPENAI: undefined as any,
        COURSE_CODE_USE_GEMINI: undefined as any,
        COURSE_CODE_USE_BEDROCK: undefined as any,
        COURSE_CODE_USE_VERTEX: undefined as any,
        COURSE_CODE_USE_FOUNDRY: undefined as any,
      },
    })
    if (error) {
  ...
  function deleteGithubProvider(): string | null {
    ...
    const { error } = updateSettingsForSource('userSettings', {
      env: {
        COURSE_CODE_USE_GITHUB: undefined as any,
