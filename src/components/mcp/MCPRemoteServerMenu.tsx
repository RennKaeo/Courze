...
export function MCPRemoteServerMenu({
  ...
  const handleClaudeAIAuthComplete = React.useCallback(async () => {
    ...
    try {
      ...
      if (success) {
        ...
        onComplete?.('Authentication successful, but server still requires authentication. You may need to manually restart Course Code.');
      } else {
  ...
  const handleClaudeAIAuth = React.useCallback(async () => {
    ...
    if (orgUuid && server.config.type === 'claudeai-proxy' && server.config.id) {
      ...
      const productSurface = encodeURIComponent(process.env.COURSE_CODE_ENTRYPOINT || 'cli');
