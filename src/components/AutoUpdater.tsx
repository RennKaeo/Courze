...
export function AutoUpdater({
  ...
  const checkForUpdates = React.useCallback(async () => {
    ...
    if (!isDisabled && currentVersion && latestVersion && !gte(currentVersion, latestVersion) && !shouldSkipVersion(latestVersion)) {
      ...
      if (installStatus === 'success') {
        logEvent('courze_auto_updater_success', {
        ...
        logEvent('courze_auto_updater_fail', {
          fromVersion: currentVersion as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
  ...
  return <Box flexDirection="row" gap={1}>
      ...
      {(autoUpdaterResult?.status === 'install_failed' || autoUpdaterResult?.status === 'no_permissions') && <Text color="error" wrap="truncate">
          ✗ Auto-update failed &middot; Try <Text bold>course doctor</Text> or{' '}
          <Text bold>
            {hasLocalInstall ? `cd ~/.course/local && npm update ${MACRO.PACKAGE_URL}` : `npm i -g ${MACRO.PACKAGE_URL}@latest`}
          </Text>
        </Text>}
    </Box>;
}
