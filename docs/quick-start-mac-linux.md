# Course Code Quick Start for macOS and Linux
...
## 2. Install Course Code

```bash
npm install -g @renskaeo/courze@latest
```
On Arch Linux, you can alternatively install Course Code via the community-maintained [AUR package](https://aur.archlinux.org/packages/course):
...
If you prefer to keep your keys in a `.env` file instead of exporting them individually, note that Course Code does not load `.env` files automatically. You must explicitly pass it:
...
- if same-session chat history appears missing, verify the active `CONTEXT`
  value with `ollama ps`; Course Code requests 32K by default
...
## 7. Uninstalling Course Code

**Via npm:**
```bash
npm uninstall -g @renskaeo/courze
```
