...
Course Code can connect to LiteLLM through LiteLLM's OpenAI-compatible proxy.

## Overview

LiteLLM is an open-source LLM gateway that provides a unified API to 100+ model providers. By running the LiteLLM Proxy, you can route Course Code requests through LiteLLM to access any of its supported providers — all while using Course Code's existing OpenAI-compatible provider path.
...
## 2. Point Course Code to LiteLLM
...
- LiteLLM's OpenAI-compatible endpoint accepts the same request format as OpenAI, so Course Code works without custom request shaping.
- Course Code discovers LiteLLM model context from `/v1/models` when LiteLLM exposes `context_length`, `context_window`, `max_model_len`, or `max_input_tokens`, including under `model_info`.
- You can switch between any provider configured in LiteLLM by simply changing the `OPENAI_MODEL` value — no need to reconfigure Course Code.

### Context window detection

For custom LiteLLM aliases, add context metadata to each model entry when the
upstream model supports a larger window than Course Code's fallback:
