...
This page documents optional PowerShell helper functions for launching Course Code on Windows after a global npm install.
...
The helpers use the installed `course` CLI command. They do not require a source checkout and do not call source-only `bun run scripts/*.ts` entrypoints.
...
~~~powershell
$packageRoot = Join-Path (npm root -g) "@renskaeo/courze"
$aliases = Join-Path $packageRoot "scripts\windows\course-aliases.ps1"

if (-not (Test-Path $aliases)) {
  throw "Alias script not found at $aliases. Update or reinstall @renskaeo/courze."
}
...
### Launch Course Code using the installed CLI
...
The environment overrides are scoped to that single `course` invocation. A later plain `oc` call returns to normal installed CLI behavior and saved-provider-profile behavior.
...
This opens the provider manager through the installed Course Code CLI.
...
| Command | Purpose |
| --- | --- |
| `oc` | Launch Course Code using the installed CLI and saved/default behavior |
