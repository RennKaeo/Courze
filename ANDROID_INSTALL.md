# Course Code on Android (Termux)

A complete guide to running Course Code on Android using Termux + proot Ubuntu.
...
Course Code requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 6 — Build Course Code

```bash
cd /data/data/com.termux/files/home/course
bun run build
```

You should see:
```
✓ Built course v0.1.6 → dist/cli.mjs
```
...
```bash
echo 'export COURSE_CODE_USE_OPENAI=1' >> ~/.bashrc
...
Both were tested and fail due to Course Code's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the Course Code repo.
