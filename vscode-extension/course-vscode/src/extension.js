...
async function buildLaunchAzureEnv(configured) {
  ...
  if (!endpoint || !deployment) {
    void vscode.window.showWarningMessage(
      'Course Code Azure chat is enabled but endpoint or deployment is missing. Run "Course Code: Configure Azure / Foundry Chat" or set course.azure.* in settings.',
    );
    return env;
  }
  if (!apiKey) {
    void vscode.window.showWarningMessage(
      'Course Code Azure chat is enabled but no API key is set. Use "Course Code: Set Azure / Foundry API Key" or course.azure.apiKey (not recommended).',
    );
    return env;
  }
...
async function setAzureApiKey(context) {
  const key = await vscode.window.showInputBox({
    title: 'Course Code — Azure / Foundry API key',
  ...
  void vscode.window.showInformationMessage('Course Code Azure / Foundry API key saved to Secret Storage.');
}
...
async function clearAzureApiKey(context) {
  ...
  void vscode.window.showInformationMessage('Course Code Azure / Foundry API key removed from Secret Storage.');
}
...
async function configureAzureChat(context) {
  const cfg = vscode.workspace.getConfiguration('course');
  const target = vscode.ConfigurationTarget.Global;

  const endpoint = await vscode.window.showInputBox({
    title: 'Course Code — Azure / Foundry API endpoint',
  ...
  const apiVersion = await vscode.window.showInputBox({
    title: 'Course Code — Azure API version',
  ...
  const deployment = await vscode.window.showInputBox({
    title: 'Course Code — Azure deployment / model',
  ...
  void vscode.window.showInformationMessage(
    'Course Code Azure / Foundry chat saved. Launch Course Code from the Control Center or command palette.',
  );
}
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('course');
  const launchCommand = configured.get('launchCommand', 'course');
  const terminalName = configured.get('terminalName', 'Course Code');
...
async function launchCourse Code(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `Course Code command not found: ${executable}. Install it with: npm install -g @renskaeo/courze@latest`,
      'Open Setup Guide',
      'Open Repository',
    );
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch Course Code because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="Course Code wordmark">Open<span class="wordmark-accent">Claude</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the upstream Course Code project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and Course Code commands quickly.</span>
            </button>
...
class CourseCodeControlCenterProvider {
  ...
  async resolveWebviewView(webviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          await launchCourse Code();
          break;
        case 'launchRoot':
          await launchCourse Code({ requireWorkspace: true });
          break;
        case 'openProfile':
        ...
        case 'commands':
          ...
          break;
        case 'azureSettings':
          await vscode.commands.executeCommand('workbench.action.openSettings', 'course.azure');
          break;
        case 'refresh':
        default:
          break;
...
function activate(context) {
  ...
  statusBarItem.text = '$(comment-discussion) Course Code';
  statusBarItem.tooltip = 'Open Course Code Chat';
  statusBarItem.command = 'course.openChat';
  statusBarItem.show();

  chatController.onDidChangeState((state) => {
    switch (state) {
      case 'streaming':
        statusBarItem.text = '$(sync~spin) Course Code';
        statusBarItem.tooltip = 'Course Code is generating...';
        break;
      case 'connected':
        statusBarItem.text = '$(comment-discussion) Course Code';
        statusBarItem.tooltip = 'Course Code connected';
        break;
      default:
        statusBarItem.text = '$(comment-discussion) Course Code';
        statusBarItem.tooltip = 'Open Course Code Chat';
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('course.start', async () => {
    await launchCourse Code();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'course.startInWorkspaceRoot',
    async () => {
      await launchCourse Code({ requireWorkspace: true });
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'course.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('course.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.course');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('course.newChat', () => {
    chatController.stopSession();
    chatController.broadcast({ type: 'session_cleared' });
  });

  const openChatCommand = vscode.commands.registerCommand('course.openChat', () => {
    chatPanelManager.openPanel();
  });

  const resumeSessionCommand = vscode.commands.registerCommand('course.resumeSession', async () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('course.abortChat', () => {
    chatController.abort();
  });

  const setAzureApiKeyCommand = vscode.commands.registerCommand('course.setAzureApiKey', async () => {
    await setAzureApiKey(context);
  });

  const clearAzureApiKeyCommand = vscode.commands.registerCommand('course.clearAzureApiKey', async () => {
    await clearAzureApiKey(context);
  });

  const configureAzureChatCommand = vscode.commands.registerCommand('course.configureAzureChat', async () => {
    await configureAzureChat(context);
  });

  const openAzureSettingsCommand = vscode.commands.registerCommand('course.openAzureSettings', async () => {
    await vscode.commands.executeCommand('workbench.action.openSettings', 'course.azure');
  });

  // ── Register providers ──
  const controlCenterProviderReg = vscode.window.registerWebviewViewProvider(
    'course.controlCenter',
    provider,
  );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('course')) {
        refreshProvider();
      }
    }),
