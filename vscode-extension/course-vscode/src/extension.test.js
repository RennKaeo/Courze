...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'course',
    launchCommand: 'course --project-aware',
    terminalName: 'Course Code',
    shimEnabled: false,
    workspaceFolder: '/workspace/course/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/course/very/long/path/example-project',
    launchCwdLabel: '/workspace/course/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/course/very/long/path/example-project/.courzerc.json',
    workspaceProfilePath: '/workspace/course/very/long/path/example-project/.courzerc.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the Course Code wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/course\/very\/long\/path\/example-project"[^>]*>\/workspace\/course\/very\/long\/path\/example-project<\//,
  );
});
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/course/src/panels/control-center.js',
      workspacePath: '/workspace/course',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/course/src/panels',
      projectAwareCwdLabel: '/workspace/course/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/course',
      workspaceRootCwdLabel: '/workspace/course',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/course',
      activeFilePath: '/workspace/course/src/panels/control-center.js',
      workspacePath: '/workspace/course',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/course',
      projectAwareCwdLabel: '/workspace/course',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/course/src/panels',
      launchCwdLabel: '/workspace/course/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/course\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/course/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/course',
      launchCwdLabel: '/workspace/course',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/course/);
  assert.match(html, /Same workspace-root target as Launch Course Code because the relative command resolves from the workspace root · \/workspace\/course/);
});
