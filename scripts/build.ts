/**
 * Course Code build script — bundles the TypeScript source into a single
...
result = await Bun.build({
  ...
  define: {
    ...
    'MACRO.ISSUES_EXPLAINER':
      JSON.stringify('report the issue at https://github.com/RennKaeo/Courze/issues'),
    'MACRO.FEEDBACK_CHANNEL':
      JSON.stringify('https://github.com/RennKaeo/Courze/issues'),
    'MACRO.PACKAGE_URL': JSON.stringify('@renskaeo/courze'),
        ...
        build.onLoad(
          ...
          (args) => {
            ...
            return {
              contents: `
const noop = () => null;
;(globalThis.__courseStubMarkers ??= []).push(${marker});
export default noop;
${exports}
`,
              loader: 'js',
...
if (!result.success) {
  ...
  console.log(`✓ Built course v${version} → dist/cli.mjs`)
}
...
if (result?.success) {
  ...
  // differs only by the repo-root prefix (`/home/ubuntu/.../course` locally
  // vs `/home/runner/work/course/course` on CI). canonicalStub() keys
